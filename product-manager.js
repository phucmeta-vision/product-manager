...
// Danh sách môn học (đọc từ localStorage key "lists", do trang category-manager quản lý).
// Biến toàn cục này được cập nhật mỗi khi renderSubjectFilter() chạy, để renderLesson()
// và handleEdit() có thể tra cứu tên môn học theo categoryId mà không cần đọc lại localStorage.
let categoryLists = JSON.parse(localStorage.getItem("lists")) || [];

/**
 * Hàm getCategoryLabel - Trả về thông tin hiển thị (tên + trạng thái) của một môn học theo id.
 * Xử lý 3 trường hợp:
 *   1. Môn học tồn tại và đang hoạt động → trả về tên bình thường
 *   2. Môn học tồn tại nhưng đã ngừng hoạt động → trả về tên kèm ghi chú
 *   3. Môn học không còn tồn tại (đã bị xóa bên category-manager) → trả về cảnh báo "Đã xóa"
 * Việc này giúp bảng bài học luôn phản ánh đúng dữ liệu môn học hiện tại, kể cả khi
 * môn học đã bị đổi trạng thái hoặc xóa ở trang Quản lý môn học.
 */
const getCategoryLabel = (categoryId) => {
    const category = categoryLists.find(c => c.id === categoryId);

    if (!category) {
        return { text: "⚠ Môn học đã bị xóa", className: "category-deleted" };
    }

    if (category.status !== "active") {
        return { text: `${category.name} (Ngừng hoạt động)`, className: "category-inactive" };
    }

    return { text: category.name, className: "" };
};
...
selectAllCheckbox.addEventListener("change", () => {
    ...
    currentIds.forEach(id => {
        ...
        if (item) item.status = isChecked ? "done" : "notDone";
    });
...
const renderLesson = (data) => {
    ...
    paginatedData.forEach((item) => {
        ...
        let statusClass = item.status === "done" ? "st-active-status" : "st-inactive-status";

        // [ĐIỀU KIỆN] Xác định văn bản hiển thị cho trạng thái
        let statusText  = item.status === "done" ? "Đã hoàn thành" : "Chưa hoàn thành";

        // Lấy thông tin hiển thị môn học (tên / ngừng hoạt động / đã bị xóa) theo categoryId
        let categoryInfo = getCategoryLabel(item.categoryId);

        // Xây dựng một hàng <tr> bằng template literal
        // - class "completed-row" chỉ được thêm nếu status === "done" (điều kiện ba ngôi)
        // - checkbox: checked nếu status === "done", khi thay đổi gọi handleToggleStatus(id)
        ...
        html += `
        <tr class="${item.status === "done" ? "completed-row" : ""}">
            <td><input type="checkbox" ${item.status === "done" ? "checked" : ""} onchange="handleToggleStatus(${item.id})"></td>
            <td>${item.name}</td>
            <td class="${categoryInfo.className}">${categoryInfo.text}</td>
            <td>${item.time}</td>
            ...
            <td>
                <button class="btn-icon btn-icon-del" onclick="handleDelete(${item.id})" title="Xóa">
                    <img src="../assets/icons/Button.png" alt="Xóa">
                </button>
                <button class="btn-icon btn-icon-edit" onclick="handleEdit(${item.id})" title="Sửa">
                    <img src="../assets/icons/Button (1).png" alt="Sửa">
                </button>
            </td>
        </tr>
        `;
...
const filterAndSearchLessons = () => {
    ...
    if (subjectValue !== "all") {
        filteredLessons = filteredLessons.filter(item => String(item.categoryId) === subjectValue);
    }
...
btnAddLesson.addEventListener("click", () => {
    ...
    let status     = "notDone";                           // Mặc định bài học mới = chưa hoàn thành
    let categoryId    = subjectNameSelectElement.value;       // Môn học được chọn
    ...
    lessons.push({
        ...
        categoryId:Number(categoryId)  // Shorthand property: categoryId: categoryId
    });
...
const handleEdit = (id) => {
    ...
    // [ĐỒNG BỘ DỮ LIỆU] Dropdown edit (subjectNameEditElement) mặc định chỉ chứa các môn
    // ĐANG HOẠT ĐỘNG (xem renderSubjectFilter). Nếu môn học của bài học này đã bị chuyển
    // sang "Ngừng hoạt động" hoặc đã bị xóa hẳn ở trang category-manager, option tương ứng
    // sẽ không tồn tại trong dropdown → gán value sẽ thất bại (rơi về rỗng) và bấm "Lưu"
    // sẽ bị báo lỗi "Không được để trống" một cách sai lệch.
    // Để tránh việc đó, ta tự thêm một <option> tạm cho đúng môn học hiện tại của bài học
    // (kèm ghi chú trạng thái) trước khi gán giá trị, đảm bảo modal luôn hiển thị đúng dữ liệu.
    const currentCategory = categoryLists.find(c => c.id === lesson.categoryId);
    if (!currentCategory) {
        // Môn học đã bị xóa hẳn khỏi hệ thống
        const opt = document.createElement("option");
        opt.value = lesson.categoryId;
        opt.textContent = "⚠ Môn học đã bị xóa - vui lòng chọn môn học khác";
        subjectNameEditElement.appendChild(opt);
    } else if (currentCategory.status !== "active") {
        // Môn học còn tồn tại nhưng đang ở trạng thái ngừng hoạt động
        const opt = document.createElement("option");
        opt.value = currentCategory.id;
        opt.textContent = `${currentCategory.name} (Ngừng hoạt động)`;
        subjectNameEditElement.appendChild(opt);
    }

    subjectNameEditElement.value   = lesson.categoryId; // Môn học đang được chọn
...
btnEditSaveElement.addEventListener("click", () => {
    ...
    let newCategoryId   = Number(subjectNameEditElement.value);       // Môn học mới
    ...
    if (itemEdit) {
        ...
        itemEdit.categoryId = Number(newCategoryId);
    }
 ...
 * [ĐIỀU KIỆN] Nếu đang "done" → chuyển sang "notDone" (và ngược lại).
...
const renderSubjectFilter = () => {
    // Đọc danh sách môn học từ trang Quản lý môn học (lưu chung localStorage, key "lists")
    // Gán vào biến TOÀN CỤC categoryLists để renderLesson()/handleEdit() dùng chung,
    // đảm bảo luôn lấy đúng dữ liệu môn học mới nhất mỗi khi hàm này được gọi lại.
    categoryLists = JSON.parse(localStorage.getItem("lists")) || [];

    // Chỉ những môn "Đang hoạt động" mới cho phép chọn khi thêm/sửa bài học
    let activeSubjects = categoryLists.filter(item => item.status === "active");

    // Dropdown "Lọc theo môn học" ngoài bảng: cho lọc theo TẤT CẢ môn (kể cả đã ngừng
    // hoạt động), vì có thể vẫn còn bài học cũ gắn với môn đó
    let filterHtml = `<option value="all">Lọc theo môn học</option>`;
    categoryLists.forEach(subject => {
        filterHtml += `<option value="${subject.id}">${subject.name}</option>`;
    });
    filterSubjectElement.innerHTML = filterHtml;

    let selectHtml = `<option value="">-- Chọn môn học --</option>`;
    activeSubjects.forEach(subject => {
        selectHtml += `<option value="${subject.id}">${subject.name}</option>`;
    });
    subjectNameSelectElement.innerHTML = selectHtml;
    subjectNameEditElement.innerHTML   = selectHtml;
};


// ============================================================
// PHẦN 16: ĐỒNG BỘ DỮ LIỆU GIỮA CÁC TAB/TRANG (localStorage "storage" event)
// ============================================================

/**
 * Mặc định, localStorage không tự đẩy thay đổi sang các tab/trang khác đang mở.
 * Sự kiện "storage" của trình duyệt sẽ tự bắn ra ở các tab KHÁC (không phải tab vừa ghi)
 * mỗi khi một key trong localStorage bị thay đổi.
 *
 * Nhờ đó, nếu người dùng đang mở product-manager.html và đồng thời sửa/xóa môn học
 * ở tab category-manager.html, hoặc ngược lại sửa lessons từ tab khác, bảng bài học
 * ở đây sẽ tự cập nhật lại ngay mà không cần F5 trang.
 */
window.addEventListener("storage", (e) => {
    // [ĐIỀU KIỆN] key "lists" thay đổi (môn học bị thêm/sửa/xóa ở category-manager)
    // → cập nhật lại dropdown môn học + tên môn học hiển thị trong bảng bài học
    if (e.key === "lists") {
        renderSubjectFilter();
        filterAndSearchLessons();
    }

    // [ĐIỀU KIỆN] key "lessons" thay đổi (vd: tab khác cũng đang mở product-manager.html)
    // → đọc lại dữ liệu bài học mới nhất và render lại bảng
    if (e.key === "lessons") {
        lessons = JSON.parse(e.newValue) || [];
        filterAndSearchLessons();
    }
});
